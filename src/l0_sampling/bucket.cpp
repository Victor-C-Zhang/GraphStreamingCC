...
bool Bucket_Boruvka::is_good(const vec_t& n, const ubucket_t& large_prime, const XXH64_hash_t& bucket_seed, const ubucket_t& r) const {
...
void Bucket_Boruvka::update(const Update& update, const ubucket_t& large_prime, const ubucket_t& r) {
  ...
  c += large_prime;
  auto const cd = static_cast<bucket_prod_t>(update.delta) *
        static_cast<bucket_prod_t>(PrimeGenerator::powermod(r,update.index + 1,large_prime));
  auto const cdm = cd % static_cast<bucket_prod_t>(large_prime);
  c = (static_cast<bucket_prod_t>(c) + cdm) % large_prime;
}
